with Ada.Text_IO; use Ada.Text_IO;
with Geometria; use Geometria;
with IO; use IO;
with Ada.Numerics.Float_Random; use Ada.Numerics.Float_Random;
procedure Main is
   --Figuras!
   --Va de lo siguiente
   --Crear un paquete
   --Tenemos la clase abstracta figura con el metodo abstracto Calcular_Area
   --Herdan el cuadrado y el circulo   --El rectacgulo tiene base y altura
   --El circulo tiene radio   --No se olviden de los getters!! y los constructores!!
   --Las subclases implementan el metodo Calcular_Area
   --En el main generar un vector de 10 figuras en forma aleatoria que
   --tenga circulos y rectangulos con tamaño tambien aleatorio
   --Mostrar la sumatoria del area de todas las figuras una vez cargado el array
    type Array_Formas is array (Positive range <>) of Figura;

    Num_Formas : Integer;

      Aleatorios : Array_Formas(1..10) := (others=>null);
      G : Generator;
      Azar : Uniformly_Distributed;
   begin


end Main;
